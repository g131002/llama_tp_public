...
class LlamaMLP(nn.Module):
    def __init__(self, config):
        ...
        self.intermediate_size = config.intermediate_size // tp
    ...
    def forward(self, x):
        ...
        down_proj = all_reduce(down_proj)
...
class LlamaAttention(nn.Module):
    ...
    def __init__(self, config: LlamaConfig, layer_idx: Optional[int] = None):
        ...
        assert config.hidden_size % tp == 0

        self.attention_dropout = config.attention_dropout
        self.hidden_size = config.hidden_size
        self.num_heads = config.num_attention_heads // tp
        self.head_dim = self.hidden_size // self.num_heads // tp
        self.num_key_value_heads = config.num_key_value_heads // tp
        self.num_key_value_groups = self.num_heads // self.num_key_value_heads
        self.max_position_embeddings = config.max_position_embeddings
        self.rope_theta = config.rope_theta
        self.is_causal = True

        if (self.head_dim * self.num_heads * tp) != self.hidden_size:
            raise ValueError(
...
class LlamaSdpaAttention(LlamaAttention):
    ...
    def forward(
        ...
        **kwargs,
        ...
        attn_output = all_reduce(attn_output)
